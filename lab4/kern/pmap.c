...
mem_init(void)
{
	...
	// kern_pgdir wrong.
	cr4 = rcr4();	//set the PSE bit of cr4 to 1
	cr4 |= CR4_PSE;
	lcr4(cr4);
...
page_init(void)
{
	...
	size_t i;
	page_free_list = NULL;
	size_t used_pages = PADDR(boot_alloc(0)) / PGSIZE;
	size_t mpentry_page = PGNUM(MPENTRY_PADDR);
	for (i = 0; i < npages; i++) {
		if ((i > 0 && i < npages_basemem && i != mpentry_page) || i > used_pages){ 	//free
			pages[i].pp_ref = 0;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pgdir[PDX(va)] & PTE_P) {
		pgdir_entry = (pte_t *) KADDR(PTE_ADDR(pgdir[PDX(va)]));
		return &pgdir_entry[PTX(va)];
	}
	else {		//not existed
		if (!create)
			return NULL;
