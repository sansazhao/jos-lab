...
// helper function: only print number with recursion
static void
printnum_only(void (*putch)(int, void*), void *putdat, unsigned long long num, unsigned base)
{
	if (num >= base) {
		printnum_only(putch, putdat, num / base, base);
        }
	 ...
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	printnum_only(putch, putdat, num, base);
	if (padc == '-') {
		while (width-- > 0)
			putch(' ', putdat);
	}
