...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (!e->env_ipc_recving) {
		// challenge: block the syscall in kern, wait for recving
		curenv->env_ipc_sending = 1;
		curenv->env_ipc_to = envid;
		curenv->env_ipc_value_block = value;
		curenv->env_ipc_srcva = srcva;
		curenv->env_ipc_perm_block = perm;
	...
	e->env_tf.tf_regs.reg_eax = 0;
	e->env_ipc_perm = perm;			// lab6 httpd check perm!!
	return 0;
}
...
sys_ipc_recv(void *dstva)
{
	...
	for (int i = 0; i < NENV; ++i) {
		if (envs[i].env_ipc_sending && envs[i].env_ipc_to == curenv->env_id 
			&& envs[i].env_status == ENV_NOT_RUNNABLE) {
			e = &envs[i];
			e->env_ipc_sending = 0;
			e->env_status = ENV_RUNNABLE;
			if (e->env_ipc_srcva < (void *)UTOP) {	//multiple senders, may some invalid 
				if (PGOFF(e->env_ipc_srcva) || e->env_ipc_perm_block & (~PTE_SYSCALL) 
				|| (e->env_ipc_perm_block & (PTE_U | PTE_P)) != (PTE_U | PTE_P))
					continue;
				if (!(pp = page_lookup(e->env_pgdir, e->env_ipc_srcva, &pte)))
					continue;
				if ((e->env_ipc_perm_block & PTE_W) && (!(*pte & PTE_W)))
					continue;
				if (dstva < (void *)UTOP && 	//receiver share 
					(r = page_insert(curenv->env_pgdir, pp, dstva, e->env_ipc_perm_block)) < 0)
					continue;
			}
			// find a valid send, set the curenv  (todo: multiple envs may have bug?)
			curenv->env_ipc_from = e->env_id;
			curenv->env_ipc_value = e->env_ipc_value_block;
			curenv->env_ipc_perm = e->env_ipc_perm_block;
			return 0;	// return to the blocked sender
		}
	} 
