...
copy_shared_pages(envid_t child)
{
	...
	// loop through all page table entries in the current process (just like fork did), 
	...
	for (uintptr_t addr = UTEXT; addr < UTOP; addr += PGSIZE) {	
		if ((uvpd[PDX(addr)] & PTE_P) && ((uvpt[PGNUM(addr)] & (PTE_P|PTE_SHARE)) == (PTE_P|PTE_SHARE)) 
			...
			if ((r = sys_page_map(0, (void *)addr, child, (void *)addr, PTE_SYSCALL)) < 0)
				panic("copy_shared_page: %e\n", r);
		}
	}
	return 0;
}
